...
- **Population dynamics** — growth curves (none / linear / exponential / logistic)
  ...
  neighbor and are guests on the day of their first actual transfer (normally
  the join day). Leavers spend the configurable exit-spend share of their
...
- **Personas** — a fully editable list (add/remove up to 8, named freely;
  ...
  range, initial balance weight, decay response, recirculation share and a
...
- **Decay-response behaviour** — each persona has a `decay response` elasticity:
  transfer frequency scales with the annualized decay rate (capped at 5x), with
  an extra spending spike on the eve of each decay application (Woergl-style
  "spend it before it shrinks"). Set to 0 for decay-indifferent agents.
...
- Decay-response constants: the frequency boost caps at 5x; the eve-of-decay
  spike is `response x per-application loss x 10`. Both are heuristics — tune
